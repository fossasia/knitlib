...
# uncomment this to use new, more precise method of finding patterns in the dat file,
# based on kh940 format documentation from https://github.com/stg/knittington/blob/master/doc/kh940_format.txt
# (should work with all kh930 and kh940 models)
methodWithPointers = True
            ...
            if flag != 0:
                ...
                if patternNumber:
                    if patternNumber == patno:
                        patlist.append(
                            {'number': patno, 'stitches': stitches, 'rows': rows, 'memo': memoff, 'pattern': patoff,
                             'pattend': pptr})
                else:
                    patlist.append(
                        {'number': patno, 'stitches': stitches, 'rows': rows, 'memo': memoff, 'pattern': patoff,
